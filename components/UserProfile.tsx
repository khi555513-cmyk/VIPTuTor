...
import { User, Mail, Phone, Target, Camera, Save, CreditCard, LogOut, Crown, Star, CheckCircle, Zap, Shield, ShoppingCart, Loader2, MessageCircle, AlertTriangle, Key, Edit2, ShieldCheck, RefreshCw } from 'lucide-react';
    ...
    <div className="h-full bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-[#0f172a] rounded-2xl shadow-xl p-4 md:p-6 text-white border border-indigo-500/30">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div className="flex items-center gap-4">
                   <div className="w-12 h-12 bg-indigo-500/20 rounded-xl flex items-center justify-center border border-indigo-500/40">
                      <ShieldCheck className="w-6 h-6 text-indigo-400" />
                   </div>
                   <div>
                      <h3 className="font-bold text-lg">Xác thực Gemini API</h3>
                      <p className="text-slate-400 text-xs">Trạng thái kết nối: <span className="text-green-400 font-bold">Secure (AES-256)</span></p>
                   </div>
                </div>
                <button 
                  onClick={onManageApiKey}
                  className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/40 active:scale-95"
                >
                   <RefreshCw className="w-4 h-4" /> Quản lý API Key
                </button>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white/5 border border-white/10 p-3 rounded-lg flex items-center gap-3">
                   <div className="w-8 h-8 rounded bg-white/10 flex items-center justify-center"><Shield className="w-4 h-4 text-slate-400" /></div>
                   <div className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Key Storage: Secure Vault</div>
                </div>
                <div className="bg-white/5 border border-white/10 p-3 rounded-lg flex items-center gap-3">
                   <div className="w-8 h-8 rounded bg-white/10 flex items-center justify-center"><Key className="w-4 h-4 text-slate-400" /></div>
                   <div className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Encrypted in Browser</div>
                </div>
             </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-6">
             ...
             {profile.accountTier !== 'basic' && ( <div className="mt-4 flex items-center justify-end text-sm text-gray-500"> <Shield className="w-4 h-4 mr-1" /> Hạn sử dụng: <strong className="ml-1 text-gray-800"> {profile.subscriptionExpiry ? new Date(profile.subscriptionExpiry).toLocaleDateString('vi-VN') : "Vĩnh viễn"} </strong> </div> )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="px-4 md:px-6 pb-6 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div> <label className="block text-sm font-semibold text-gray-700 mb-1.5">Họ và tên</label> <input type="text" disabled={!isEditing} value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full p-2.5 border border-gray-200 rounded-lg bg-gray-50 disabled:bg-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /> </div>
                <div> <label className="block text-sm font-semibold text-gray-700 mb-1.5">Mục tiêu học tập</label> <select disabled={!isEditing} value={formData.target || 'Giao tiếp cơ bản'} onChange={(e) => setFormData({...formData, target: e.target.value})} className="w-full p-2.5 border border-gray-200 rounded-lg bg-gray-50 disabled:bg-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none"> <option>Giao tiếp cơ bản</option> <option>IELTS 6.0+</option> <option>IELTS 7.0+</option> <option>IELTS 8.0+</option> <option>THPT Quốc Gia 9+</option> </select> </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-6">
             ...
             <p className="text-xs text-center text-gray-400 mt-6 italic"> * Sau khi thanh toán qua Zalo, bạn sẽ nhận được Mã Kích Hoạt để nhập vào ô ở trên. </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 md:p-6 mb-8">
               ...
               <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <p className="text-xs text-gray-500"> Xóa toàn bộ dữ liệu chat và cài đặt. Hành động này không thể hoàn tác. </p>
